...
export default function Estantes({ livros, handleEstadoLivro }) {
  ...
  const retornaLivrosParaExibir = () => {
    if (activeTab === "todos") { return livros }
    if (activeTab === "estouLendo") { return livros.filter(livro => livro.estouLendo) }
    if (activeTab === "queroLer") { return livros.filter(livro => livro.queroLer) }
    if (activeTab === "jaLido") { return livros.filter(livro => livro.jaLido) }
  }
