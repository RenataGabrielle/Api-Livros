...
export default function MenuEstantes({ setActiveTab }) {
    ...
    return (
        <div className="menuEstantes">
            <button
                className={`buttonMenuEstantes ${buttonActive === "todos" ? "active" : ""}`}
                onClick={() => handleAtivarMenu("todos")}
            >
                Todos
            </button>
            <button
                className={`buttonMenuEstantes ${buttonActive === "estouLendo" ? "active" : ""}`}
                onClick={() => handleAtivarMenu("estouLendo")}
            >
                Estou Lendo
            </button>
            <button
                className={`buttonMenuEstantes ${buttonActive === "queroLer" ? "active" : ""}`}
                onClick={() => handleAtivarMenu("queroLer")}
            >
                Quero Ler
            </button>
            <button
                className={`buttonMenuEstantes ${buttonActive === "jaLido" ? "active" : ""}`}
